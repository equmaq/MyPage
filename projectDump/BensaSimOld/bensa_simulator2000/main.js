Price95 = (Math.random() * (2.34 - 1.68) + 1.68).toFixed(3);
Price98 = (Math.random() * (2.34 - 1.68) + 1.68).toFixed(3);
PriceDiesel = (Math.random() * (2.34 - 1.68) + 1.68).toFixed(3);
PriceMehu = 0.89;
Selected = 0;
crscan = "null";
bonus = "false";
...
function insert() {
  if (pumpingprocess == "1") {
    pumpingprocess = "2";
    document.getElementById("Pump").style.backgroundColor = "orangered";
  }
}
...
function pump() {
  ...
  if (pumped * SelPrice >= finalAmoutWanted) {
    document.getElementById("Finish").style.backgroundColor = "#16A012";
    pumpingprocess = "3";
  }
}
